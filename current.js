...
function updateView() {
    ...
    // Handle Durham/Multi-match cases
    ...
    if (mappedIds) {
        ...
        // Also add fuzzy name matches that weren't in the mapped city/zip
...
function selectStore(feature) {
    ...
    // If we're searching, ensure the selected store is in the sidebar subset
    if (currentSearchTerm) {
...
function renderList(features) {
    ...
    if (sorted.length === 0) {
        locationListEl.innerHTML = `<div class="empty-state"><p>No locations found for "<strong>${currentSearchTerm}</strong>"</p></div>`;
...
function initGeolocation() {
    ...
    navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
            console.log("Geolocation silent fallback:", err.message);
...
map.on('load', () => {
    ...
    img.src = 'https://cdn.prod.website-files.com/6916222bad26f3b001d31303/69ddce3c73acb3156d0a807c_new-location-pin.svg';
